...
def v_chef(request):
    ...
    pedidos = PEDIDO.objects.order_by('idPedido')  # Obtener pedidos ordenados por el estado
...
def v_aceptar_pedido(request, id_pedido):
    ...
    pedido = get_object_or_404(PEDIDO, idPedido=id_pedido, estatus=1)  # Solo se pueden aceptar pedidos con estatus 'CREADO'

    pedido.estatus = int(2)
    pedido.save()
...
def v_listo_para_entrega(request, id_pedido):
    ...
    pedido = get_object_or_404(PEDIDO, idPedido=id_pedido, estatus=2)  # Solo se pueden aceptar pedidos con estatus 'ACEPTADO'

    pedido.estatus = int(3)
    pedido.save()
